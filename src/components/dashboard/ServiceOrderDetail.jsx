...
export default function ServiceOrderDetail({
  ...
  const getPriorityBadge = (priority) => {
    ...
    return <span className={`badge border ${item.color}`} style={{ fontSize: '0.7rem' }}>{item.label.toUpperCase()}</span>;
  };
        ...
        <div className="header-actions d-flex gap-2">
          {isSystemAdmin && (
            <button
              ...
              style={{ background: "var(--bs-danger)", border: "none", padding: '8px 15px', borderRadius: '8px' }}
            >
          ...
          <div className="info-card p-4 h-100">
            <div className="d-flex justify-content-between align-items-start mb-4">
              <div>
                ...
                <p className="text-dim">Aberto em: {new Date(order.created_at).toLocaleString('pt-BR')}</p>
              </div>
            ...
            {order.attachment_path && (
              <div className="attachment-section mt-4">
                <h6 className="text-primary-light text-uppercase fw-bold small mb-2">Anexo Enviado</h6>
                <a 
                  href={`${process.env.REACT_APP_API_URL}/storage/${order.attachment_path}`} 
                  target="_blank" 
                  rel="noreferrer"
                  className="d-inline-flex align-items-center p-2 rounded-3 text-decoration-none"
                  style={{ background: 'rgba(111, 66, 193, 0.1)', border: '1px dashed var(--bs-primary)', color: 'var(--bs-primary)' }}
                >
                  <i className="bi bi-file-earmark-arrow-down fs-4 me-2"></i>
                  <span>Visualizar Arquivo em Anexo</span>
                </a>
              </div>
            )}
          </div>
          ...
          <div className="info-card p-4 mb-4">
            ...
            <div className="mb-4">
              ...
              <div className="d-flex align-items-center p-2 rounded bg-dark border border-secondary">
                <div className="avatar-circle me-2" style={{ width: '32px', height: '32px', background: '#343a40', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.8rem' }}>
                   {order.user?.name?.charAt(0)}
                </div>
                <div>
                  <div className="text-white small fw-bold">{order.user?.name}</div>
                  <div className="text-dim" style={{ fontSize: '0.7rem' }}>{order.user?.email}</div>
                </div>
              </div>
            </div>
