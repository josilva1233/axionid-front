...
export default function ServiceOrderDetail({
  ...
  // ----- Carregar mensagens ao montar (NOVO) -----
  useEffect(() => {
    if (order?.id) {
      loadMessages();
    }
  ...
  // ----- Verifica se o usuário pode editar/excluir mensagem (NOVO) -----
  const canModifyMessage = (message) => {
    return isSystemAdmin || (message.user_id === currentUser?.id);
  };

  // ----- Formatação de data/hora (NOVO) -----
  ...
  // ----- Se não houver ordem, mostra loading (já existente) -----
  if (!order) {
    return (
      ...
      <div className="px-6 py-5">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            {/* --- Bloco: Status, Descrição e Anexo (já existente) --- */}
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex flex-wrap items-center justify-between gap-2">
                            ...
                            {/* Ações (editar/excluir) - somente se autorizado */}
          ...
          {/* ============ COLUNA LATERAL (Gestão) - Já existente ============ */}
